...
def init_db():
    ...
    # Check and add ref_code column if missing in older DB
    ...
    if not cursor.fetchone():
        ...
        cursor.execute("INSERT INTO users (phone, email, password, balance, status, is_admin, can_spin, ref_code) VALUES ('03000000000', 'admin@cfl.com', 'admin123', 0.00, 'Active', 1, 1, ?)", (admin_ref,))
...
def register():
    ...
    if request.method == 'POST':
        ...
        password = request.form['password']
        ref_code_input = request.form.get('ref_code', '')
        ...
        try:
            conn = sqlite3.connect(DB_PATH)
            cursor = conn.cursor()
            cursor.execute("INSERT INTO users (phone, email, password, balance, status, ref_code, referred_by) VALUES (?, ?, ?, 0.00, 'Pending', ?, ?)", 
                           (phone, email, password, my_ref_code, ref_code_input if ref_code_input else None))
            conn.commit()
            conn.close()
    ...
    content = f'''
        ...
        <form method="POST">
            ...
            <input type="text" name="ref_code" placeholder="ریفرل کوڈ (اگر ہو تو)" value="{ref_from_url}">
...
def dashboard():
    ...
    # Get total team / referrals count
    my_code = user[9] if len(user) > 9 and user[9] else ""
    team_count = 0
    if my_code:
        cursor.execute("SELECT COUNT(*) FROM users WHERE referred_by = ?", (my_code,))
        team_count = cursor.fetchone()[0]
        
    conn.close()
    
    # Calculate level based on team count (up to 50 levels / thresholds)
    # Level formula: Every 1 level per 1 referral up to level 50
    user_level = min(50, max(1, team_count // 1 + 1 if team_count > 0 else 1))
    ...
    ref_link = request.host_url + f"register?ref={my_code}" if my_code else "لینک دستیاب نہیں"
    ...
    content = f'''
        <div class="card">
            ...
            <p style="font-size: 14px; color: #34b7f1; margin-top: 8px;"><b>ٹیم ممبرز:</b> {team_count} | <b>لیول:</b> {user_level} (میکس 50)</p>
        </div>

        <div class="card" style="text-align: right; background: #182229;">
            <p style="font-size: 13px; color: #00a884; margin-bottom: 5px;">🔗 آپ کا ریفرل لنک:</p>
            <input type="text" value="{ref_link}" readonly style="font-size: 12px; background: #0b141a; color: #00a884; text-align: center;">
        </div>
...
def admin_panel():
    ...
    if request.method == 'POST':
        if 'wallet' in request.form:
            ...
            new_bal = float(request.form['new_balance'])
            cursor.execute("UPDATE users SET balance = ? WHERE phone = ?", (new_bal, target_phone))
            conn.commit()
    ...
    for u in all_users:
        users_html += f'''
            <p>موبائل: <b>{u[0]}</b> | بیلنس: ${u[1]} | اسٹیٹس: {u[2]}</p>
            <form method="POST" style="margin-top:5px; display:flex; gap:5px;">
                <input type="hidden" name="target_phone" value="{u[0]}">
                <input type="number" step="0.01" name="new_balance" placeholder="نیا بیلنس ($)" value="{u[1]}" required style="margin:0; padding:6px; font-size:12px;">
                <button type="submit" name="update_balance" value="1" style="margin:0; padding:6px; background:#00a884; font-size:12px; width:auto;">بیلنس سیٹ کریں</button>
            </form>
    ...
    content = f'''
        ...
        <div class="card" style="text-align: right;">
            <h3 style="color: #00a884;">👥 یوزرز کا بیلنس اور اسپن کنٹرول</h3>
            {users_html}
        </div>
